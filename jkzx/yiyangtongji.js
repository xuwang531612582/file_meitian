$(function(){
	...
	$('#export').bind('click',function(){
		...
		var _dept = parent.parent.parent.window.$_vo_user_info.dept[0].code;
		var _oper = parent.parent.parent.window.$_vo_user_info.code;
  ...
  var randomColor = function() {
	...
	$('#statistics').bind('click',function(){
		$('#canvas').remove();
		$('#statistics_grid').before('<canvas id="canvas"></canvas>');
		var _begin = $('#begin_date').val();
		var _end = $('#end_date').val();
		var _dept = parent.parent.parent.window.$_vo_user_info.dept[0].code;
		var _oper = parent.parent.parent.window.$_vo_user_info.code;
		var $_vo_request_data = common.f_build_request_data('report_eventdept','report_eventdept',9,[{'a_begin':{'=':_begin},'a_end':{'=':_end},'a_dept':{'=':_dept},'a_oper':{'=':_oper}}]);
		var $_chart_label = [];
		var $_data_sets = [];
		common.f_ajax_request_with_json(false,$_vo_request_data,function(_response){
			var $_td_items = [];
			var $_statistics_table = '<table id="table-1" cellspacing="0" cellpadding="0">';
            ...
            //----添加统计合计-------
				job_number += parseInt(_item['总工单']);
				on_schedule_feedback += parseInt(_item['按期反馈']);
				super_feedback += parseInt(_item['超期反馈']);
				no_feedback += parseInt(_item['未反馈']);
			//----添加统计合计结束---
				$_statistics_data += '</tr>';
            ...
            //----------
            var total = "<tr><td>合计:</td><td>"+job_number+"</td><td>"+on_schedule_feedback+"</td><td>"+super_feedback+"</td><td>"+no_feedback+"</td></tr>";
			$('#table-1').append(total);
            //----------
